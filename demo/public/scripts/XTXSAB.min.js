...
var $_$XTXAlert_C = function(str) { console.log(str);};     // alert custom function
...
function $XTXAlert_C (str) {
    
    if ($_$XTXAlert_C !== null) {
        $_$XTXAlert_C(str);
    } else {
        alert(str);
    }  
...
function CreateSecXObject() {
    var control1 = false, control2 = false, control3 = false;
    control1 = $LoadControl("02BE3F91-A9E1-4D12-A65B-1E0D500292A7", "oCert", "", false);
    control2 = $LoadControl("4F763EC7-657A-43A8-96D0-BD3AD6D5E17E", "oCrypto", "", false);
    control3 = $LoadControl("D57CD2CA-8FA1-440F-8614-B0A28F64F0BE", "oDevice", "", false);
...
function $loginCheckCertValidNotBefore(retObj)
{
    ...
    if (days > 0) {
        $XTXAlert_C("您的证书尚未生效!距离生效日期还剩" + days + "天!");
        return;
    }
...
function $loginGetPINRetryCallBack(retObj)
{
    ...
    if (retryCount > 0) {
        $XTXAlert_C("校验证书密码失败!您还有" + retryCount + "次机会重试!");
        return;
    } else if (retryCount == 0) {
        $XTXAlert_C("您的证书密码已被锁死,请联系BJCA进行解锁!");
        return;
    } else {
        $XTXAlert_C("登录失败!");
        return;
